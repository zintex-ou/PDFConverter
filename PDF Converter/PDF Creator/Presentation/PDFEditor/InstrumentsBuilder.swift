struct InstrumentsBuilder {
    static func build(viewModel: PDFEditorViewModel) -> [InstrumentsItem] {
        [
            ...
            .init(type: .reorder, icon: .property1Reorder, title: "Reorder", comletion: viewModel.reorder),
            .init(type: .share, icon: .property1Share, title: "Share", comletion: viewModel.share),
            .init(type: .extractText, icon: .property1Text, title: "Extract text", comletion: viewModel.extractText)
        ]
    }
}
